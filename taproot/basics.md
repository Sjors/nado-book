...
Now, Merkle trees are common: They're used in blocks, for file sharing, BitTorrent, etc. Using them enables you to only share the parts you need, in the context of Bitcoin that might be the script you're using to spend a coin, in the context of BitTorrent that might be a specific 2 seconds of video. The rest remains hashed, and you just add some extra data to prove that it was in that tree somewhere. In the context of BitTorrent that means your computer can receive lots of short fragments and confidently store those on your hard disk, knowing it's really a piece of the movie you're downloading, and not some garbage data.
